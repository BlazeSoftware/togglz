...
export class FeatureAdd {
  ...
  async create(e) {
    ...
    try {
      ...
      if (!existingFeature.empty) {
        this.loading = false;
        this.alertMsg = getAlertMessage('storage/document-exists');
  ...
  render() {
    return (
      <blaze-drawer position="right" dismissible ref={(drawer) => (this.drawer = drawer)}>
        <blaze-card>
          <form onSubmit={(e) => this.create(e)}>
            ...
            <blaze-card-footer>
              <button class="c-button c-button--brand c-button--block" disabled={this.loading}>
